...
const ToolsAndSkills = () => {
 ...
 const cards = [
  ...
  return (
    <section ref={sectionRef} className="tools-and-skills w-full py-24 flex justify-start items-start bg-gray-100">
        
        <div className={`${isAnimated ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-32'}   transition-all duration-700 ease-out  left w-1/3 text-right flex flex-col gap-6 h-full`}>

          <div className="flex gap-2 justify-end items-center">
            <span className="bg-lime-400 text-black size-3 rounded-full">.</span>
            <p className="text-lg font-semibold text-gray-400">Teams</p>
          </div>
        ...
        <div className="right w-2/3 grid grid-cols-2 gap-4 justify-center items-start px-8">
          
          {cards.map((card) => (
            <div key={card.id} className={`${isAnimated ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-32'} 
            m-2 card flex flex-col shadow-md rounded-lg bg-white p-10 pl-24 w-[450px] transition-all duration-1000 ease-out text-start`}>
